...
async def vplay(c: Client, m: Message):
   ...
   keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("❤️ ɢʀᴏᴜᴘ", url=f"https://t.me/{GROUP_SUPPORT}"),
    ...
    if replied:
        if replied.video or replied.document:
            loser = await replied.reply("📥 **Dᴏᴡɴʟᴏᴀᴅɪɴɢ ᴠɪᴅᴇᴏ...[ʀᴏᴄᴋs sᴇʀᴠᴇʀ](t.me/Shayri_Music_Lovers)**")
        ...
        [
            [
                ...
                await m.reply_photo(
                    photo=f"{IMG_2}",
                    caption=f"💡 **Vɪᴅᴇᴏ sᴛʀᴇᴀᴍɪɴɢ sᴛᴀʀᴛᴇᴅ.**\n\n🏷 **ɴᴀᴍᴇ:** [{songname[:10]}]({link})\n💭 **ᴄʜᴀᴛ:** `{chat_id}`\n💡 **sᴛᴀᴛᴜs:** `Playing`\n🎧 **ʀᴇǫᴜᴇsᴛ ʙʏ:** {requester}",
                    reply_markup=keyboard,
                keyboard = InlineKeyboardMarkup(
        [
            [
...
async def vstream(c: Client, m: Message):
   ...
   keyboard = InlineKeyboardMarkup(
    ...
    if len(m.command) < 2:
        await m.reply("**» ɢɪᴠᴇ ᴍᴇ ᴀ live-link/m3u8 ᴜʀʟ/ʏᴏᴜᴛᴜʙᴇ ʟɪɴᴋ ᴛᴏ sᴛʀᴇᴀᴍ.**")
    else:
        if len(m.command) == 2:
            link = m.text.split(None, 1)[1]
            Q = 720
            loser = await c.send_message(chat_id, "🔄 **Pʀᴏᴄᴇssɪɴɢ ssᴛʀᴇᴀᴍ...**")
